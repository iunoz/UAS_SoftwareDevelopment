...
const ProfilePage = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        const response = await axios.get(`uassoftwaredevelopment-production-b783.up.railway.app/api/user/${uid}`);
  ...
  useEffect(() => {
    if (showAddressModal && user && user.address) {
      ...
      axios.get(`uassoftwaredevelopment-production-b783.up.railway.app/api/ship/cities-by-province?province=${user.address.province}`)
        .then(res => {
          setCityOptions(res.data.cities);
          setAllDistricts(res.data.districts);
          setAllSubdistricts(res.data.subdistricts);
