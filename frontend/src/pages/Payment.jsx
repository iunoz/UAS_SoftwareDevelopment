...
const Payment = () => {
  ...
  useEffect(() => {
    const fetchUserAddress = async () => {
      try {
        ...
        const userRes = await axios.get(`${import.meta.env.VITE_API_URL}/user/${currentUser.uid}`);
  ...
  useEffect(() => {
    if (provinceInput.length > 1) {
        axios.get(`${import.meta.env.VITE_API_URL}/ship/search-destination?type=province&search=${provinceInput}`)
        .then(res => {
  ...
  useEffect(() => {
    if (selectedProvince) {
      axios.get(`${import.meta.env.VITE_API_URL}/ship/cities-by-province?province=${selectedProvince}`)
        .then(res => {
  ...
  const handleSaveToMainAddress = async () => {
    try {
      ...
      const res = await axios.put(`${import.meta.env.VITE_API_URL}/user/${currentUser.uid}/update-address`, {
    ...
    const fetchOngkir = async () => {
      ...
      try {
        const res = await axios.post(
          `${import.meta.env.VITE_API_URL}/ship/calculate-ongkir`,
          {},
          {
            params: {
  ...
  useEffect(() => {
    if (
      ...
      axios.get(`${import.meta.env.VITE_API_URL}/ship/cities-by-province?province=${address.province}`)
        .then(res => {
          setCityOptions(res.data.cities);
          setAllDistricts(res.data.districts);
          setAllSubdistricts(res.data.subdistricts);
  ...
  const handleMakeOrder = async () => {
    ...
    try {
      ...
      const saveRes = await axios.post(`${import.meta.env.VITE_API_URL}/payment/save-order`, orderData);
      ...
      const response = await axios.post(`${import.meta.env.VITE_API_URL}/payment/create-payment`, {
      ...
      window.snap.pay(token, {
        onSuccess: async function(result) {
          ...
          try {
            ...
            await axios.put(`${import.meta.env.VITE_API_URL}/payment/update-status/${saveRes.data.order._id}`, {
